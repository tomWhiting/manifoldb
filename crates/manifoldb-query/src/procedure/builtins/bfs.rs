//! BFS (Breadth-First Search) procedure implementation.
...
///
/// ```sql
/// CALL algo.bfs(startNode, edge_type, direction, maxDepth) YIELD node, depth, path
/// ```
///
/// # Parameters
///
...
pub struct BfsProcedure;

impl Procedure for BfsProcedure {
    fn signature(&self) -> ProcedureSignature {
        ProcedureSignature::new("algo.bfs")
            .with_description("Breadth-first search traversal from a starting node")
            .with_parameter(
    ...
    fn execute(&self, _args: ProcedureArgs) -> ProcedureResult<RowBatch> {
        Err(ProcedureError::ExecutionFailed("algo.bfs requires graph storage context".to_string()))
    }
    ...
    fn execute_with_context(
        ...
        Err(ProcedureError::ExecutionFailed(
            "BFS execution requires direct transaction access. \
...
/// Helper function to execute BFS with a transaction and return rows.
///
/// This function is intended to be called from the main manifoldb executor
/// where the transaction is available.
pub fn execute_bfs_with_tx<T: manifoldb_storage::Transaction>(
    tx: &T,
    start_id: i64,
    ...
    // Build the BFS traversal
    let mut traversal = BfsTraversal::new(start, direction).with_path_tracking();
...
mod tests {
    ...
    #[test]
    fn signature() {
        let proc = BfsProcedure;
        let sig = proc.signature();
        assert_eq!(sig.name, "algo.bfs");
