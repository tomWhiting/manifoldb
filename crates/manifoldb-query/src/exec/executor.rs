...
pub fn build_operator_tree(plan: &PhysicalPlan) -> OperatorResult<BoxedOperator> {
    match plan {
        ...
        PhysicalPlan::ShortestPath { node, input } => {
            let input_op = build_operator_tree(input)?;
            Ok(Box::new(ShortestPathOp::new((**node).clone(), input_op)))
        }
        ...
        | PhysicalPlan::CreateMaterializedView(_)
        | PhysicalPlan::DropMaterializedView(_)
        | PhysicalPlan::RefreshMaterializedView(_)
        ...
        PhysicalPlan::GraphMerge { node, input } => {
            let input_op = if let Some(input_plan) = input {
                Some(build_operator_tree(input_plan)?)
            } else {
                None
